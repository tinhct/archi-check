...
| **Upstash Redis Cache** | In-memory key-value dictionary stub simulating DB states. | Valid state caching (Success), `Promise.race()` timeout triggers (Timeout), and client errors (Error). |
| **Gemini AI API** | GoogleGenerativeAI client mock intercepting `generateContent`. | Generates schema-compliant JSON (Success) and throws 429/5xx exceptions to verify retries (Error). |
